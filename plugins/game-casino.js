...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
  ...
  cooldowns[m.sender] = Date.now()
        ...
        if (args.length < 1) return conn.reply(m.chat, '🚩 Ingresa la cantidad de ' + `*❇️ Estrellas*` + ' que deseas aportar contra' + ` *Crow Ai*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)

        if (user.limit >= count * 1) {
            user.limit -= count * 1
            if (Aku > Kamu) {
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Crow Ai* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} ❇️ Estrellas.`.trim(), m, rcanal)
            } else if (Aku < Kamu) {
                user.limit += count * 2
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Crow Ai* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} ❇️ Estrellas.`.trim(), m, rcanal)
            } else {
                user.limit += count * 1
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *CrowBot Ai* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} ❇️ Estrellas.`.trim(), m, rcanal)
            }
        } else conn.reply(m.chat, `No tienes *${formatNumber(count)} ❇️ Estrellas* para apostar!`.trim(), m, rcanal)
