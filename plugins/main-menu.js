...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let before = conn.menu.before || MenuText.before
    let header = conn.menu.header || MenuText.header
    let body = conn.menu.body || MenuText.body
    let footer = conn.menu.footer || MenuText.footer
    let after = conn.menu.after || (conn.user.jid == global.conn.user.jid ? '' : ``) + MenuText.after
    let _text = [
      before,
    ...
    await m.react('🚀')
    await conn.sendMessage(m.chat, { video: { url: pp }, gifPlayback: true, caption: text.trim(), mentions: [m.sender] })
