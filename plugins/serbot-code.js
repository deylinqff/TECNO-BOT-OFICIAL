...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
  ...
  async function serbot() {
    ...
    let conn = makeWASocket(connectionOptions)if (methodCode && !conn.authState.creds.registered) {
        if (!phoneNumber) {
            process.exit(0);
        }
    ...
    async function connectionUpdate(update) {
        ...
        if (code && code !== DisconnectReason.loggedOut && conn?.ws.socket == null) {
            ...
            if (code !== DisconnectReason.connectionClosed) {
                ...
                parent.sendMessage(m.chat, { text: "Desconectado.." }, { quoted: m })
            }
        }
    ...
    let creloadHandler = async function (restartConn) {
        ...
        conn.ev.on('connection.update', conn.connectionUpdate)conn.ev.on('creds.update', conn.credsUpdate)
        isInit = false