...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
  ...
  async function serbot() {
    ...
    if (args[0]) {
      fs.writeFileSync(`./Sesion Subbots/${authFolderB}/creds.json`, JSON.stringify(JSON.parse(Buffer.from(args[0], 'base64').toString('utf-8')), null, '\t'))
    }
    ...
    async function connectionUpdate(update) {
      ...
      if (connection === 'open') {
        ...
        await parent.sendMessage(conn.user.jid, {
          text: usedPrefix + command + ' ' + Buffer.from(fs.readFileSync(`./Sesion Subbots/${authFolderB}/creds.json`), 'utf-8').toString('base64')
        }, { quoted: m })
      }
    }
